...
const Settings = () => {
  ...
  const computedAge = calcAgeFromDob(dob);
  ...
  const save = async () => {
    setSaving(true);
    const ageToSave = computedAge ?? (user?.age ? Number(user.age) : undefined);
    try {
          ...
          <div>
            ...
            {dob && (
              <p className="text-zinc-500 text-xs mt-1 flex items-center gap-1">
                ...
                {computedAge && <span className="ml-1 text-violet-400 font-bold">· {computedAge} yrs old</span>}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest block mb-2 flex items-center gap-1.5">
              ...
              {computedAge && <span className="ml-auto text-violet-400 font-bold normal-case text-[11px]">Age: {computedAge} yrs</span>}
            </label>
            <input
              type="date"
              value={dob}
